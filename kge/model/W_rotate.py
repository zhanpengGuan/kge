...
# eval.chunk_size.
class W_RotatEScorer(RelationalScorer):
...
class W_RotatE(KgeModel):
    ...
    def __init__(
        ...
        super().__init__(
            config=config,
            dataset=dataset,
            scorer=W_RotatEScorer,
            configuration_key=self.configuration_key,
            init_for_load_only=init_for_load_only,
        )
        self._normalize_phases = self.get_option("normalize_phases")
        #clip  the weight
        self.clip = config.get("W_rotate")["clip"]
        self.W_update = False
        self.outer = False
        self.g_r = config.get("W_rotate")["golden_ratio"]
        self.init = config.get("W_rotate")["init"]
        self.require_grad_relation = config.get("W_rotate")["require_grad_relation"]
        self.require_grad_entity = config.get("W_rotate")["require_grad_entity"]
        ...
        self._weight_entity.requires_grad = config.get("W_rotate")["require_grad_entity"]
